# White Virus

> डिजिटल वायरस सौंदर्यशास्त्र पर एक इंटरैक्टिव PPSX प्रस्तुति

[简体中文](README.md) · [English](README-en.md) · [日本語](README-ja.md) · [한국어](README-ko.md) · [Español](README-es.md) · [Français](README-fr.md) · [Deutsch](README-de.md) · [Português](README-pt.md) · [Italiano](README-it.md) · [Русский](README-ru.md) · [العربية](README-ar.md) · [Türkçe](README-tr.md) · [Polski](README-pl.md) · [Nederlands](README-nl.md) · [Tiếng Việt](README-vi.md) · [ไทย](README-th.md) · [Bahasa Indonesia](README-id.md) · [Українська](README-uk.md)

---

**White Virus** "डिजिटल वायरस" सौंदर्यशास्त्र की अवधारणा पर केंद्रित एक इंटरैक्टिव प्रस्तुति है। विंडोज त्रुटि विंडो, स्क्रीन झिलमिलाहट, और पॉप-अप बमबारी जैसे दृश्य तत्वों के माध्यम से यह एक अत्यंत प्रभावशाली डिजिटल कथा अनुभव का निर्माण करता है।

## 📂 परियोजना की सामग्री

| फ़ाइल | विवरण |
|-------|-------|
| `white病毒.ppsx` | मूल PowerPoint Show फ़ाइल |

## 🚀 त्वरित आरंभ

### ⬇️ सीधे डाउनलोड करें

**डाउनलोड करें** → [white病毒.ppsx](https://github.com/apher/white-virus/releases/download/v1.0/white.ppsx)

### PPSX चलाएं
1. `white病毒.ppsx` डाउनलोड करें
2. PowerPoint या WPS में खोलें, **स्लाइड शो** मोड चुनें

## 🌐 ऑनलाइन अनुभव

> **ऑनलाइन प्लेयर (pdf.js)** → [https://white.apher.com/white-virus.html](https://white.apher.com/white-virus.html)

> **परियोजना होमपेज** → [https://white.apher.com](https://white.apher.com)

## 🎨 परियोजना के बारे में

- **मुख्य अवधारणा**: डिजिटल वायरस / ग्लिच सौंदर्यशास्त्र
- **उपयोग के मामले**: डिजिटल आर्ट प्रदर्शन, इंटरैक्टिव कथन, वेब एम्बेडिंग
- **प्रारूप**: PPSX (स्लाइड शो, संपादन योग्य नहीं)
- **गैलरी** → [https://white.apher.com](https://white.apher.com)

## 📝 कॉपीराइट

केवल सीखने और प्रदर्शन के उद्देश्यों के लिए।
White Studio · 2026

---

*Built with [OpenClaw](https://github.com/openclaw/openclaw)*
