# White Virus

> Una presentación PPSX interactiva sobre la estética del virus digital

[简体中文](README.md) · [English](README-en.md) · [日本語](README-ja.md) · [한국어](README-ko.md) · [Français](README-fr.md) · [Deutsch](README-de.md) · [Português](README-pt.md) · [Italiano](README-it.md) · [Русский](README-ru.md) · [العربية](README-ar.md) · [हिंदी](README-hi.md) · [Türkçe](README-tr.md) · [Polski](README-pl.md) · [Nederlands](README-nl.md) · [Tiếng Việt](README-vi.md) · [ไทย](README-th.md) · [Bahasa Indonesia](README-id.md) · [Українська](README-uk.md)

---

**White Virus** es una presentación interactiva centrada en la estética del "virus digital". A través de ventanas de error de Windows, parpadeos de pantalla y bombardeos de ventanas emergentes, construye una experiencia narrativa digital de alto impacto.

## 📂 Contenido del proyecto

| Archivo | Descripción |
|---------|-------------|
| `white病毒.ppsx` | Archivo PowerPoint Show original |

## 🚀 Inicio rápido

### ⬇️ Descarga directa

**Descargar** → [white病毒.ppsx](https://github.com/apher/white-virus/releases/download/v1.0/white.ppsx)

### Reproducir PPSX
1. Descarga `white病毒.ppsx`
2. Ábrelo con PowerPoint o WPS, selecciona modo **Presentación**

## 🌐 Experiencia online

> **Reproductor online (pdf.js)** → [https://white.apher.com/white-virus.html](https://white.apher.com/white-virus.html)

> **Página del proyecto** → [https://white.apher.com](https://white.apher.com)

## 🎨 Sobre el proyecto

- **Concepto central**: Virus digital / Estética glitch
- **Casos de uso**: Exhibición de arte digital, narrativa interactiva, inserción web
- **Formato**: PPSX (Presentación, no editable)
- **Galería** → [https://white.apher.com](https://white.apher.com)

## 📝 Copyright

Solo para fines de aprendizaje y exhibición.
White Studio · 2026

---

*Built with [OpenClaw](https://github.com/openclaw/openclaw)*
