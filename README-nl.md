# White Virus

> Een interactieve PPSX-presentatie over digitale virusesthetiek

[简体中文](README.md) · [English](README-en.md) · [日本語](README-ja.md) · [한국어](README-ko.md) · [Español](README-es.md) · [Français](README-fr.md) · [Deutsch](README-de.md) · [Português](README-pt.md) · [Italiano](README-it.md) · [Русский](README-ru.md) · [العربية](README-ar.md) · [हिंदी](README-hi.md) · [Türkçe](README-tr.md) · [Polski](README-pl.md) · [Tiếng Việt](README-vi.md) · [ไทย](README-th.md) · [Bahasa Indonesia](README-id.md) · [Українська](README-uk.md)

---

**White Virus** is een interactieve presentatie gericht op de esthetiek van het "digitale virus". Door Windows-foutvensters, schermflikkering en popup-bombardementen bouwt het een zeer impactvolle digitale verhaalervaring op.

## 📂 Projectinhoud

| Bestand | Beschrijving |
|---------|--------------|
| `white病毒.ppsx` | Origineel PowerPoint Show-bestand |

## 🚀 Snelstart

### ⬇️ Direct downloaden

**Downloaden** → [white病毒.ppsx](https://github.com/apher/white-virus/releases/download/v1.0/white.ppsx)

### PPSX afspelen
1. Download `white病毒.ppsx`
2. Open met PowerPoint of WPS, selecteer modus **Diashow**

## 🌐 Online ervaring

> **Online speler (pdf.js)** → [https://white.apher.com/white-virus.html](https://white.apher.com/white-virus.html)

> **Project homepage** → [https://white.apher.com](https://white.apher.com)

## 🎨 Over het project

- **Kernconcept**: Digitaal virus / Glitch-esthetiek
- **Gebruiksscenario's**: Digitale kunsttentoonstelling, interactief storytelling, webinbedding
- **Formaat**: PPSX (Diashow, niet bewerkbaar)
- **Galerij** → [https://white.apher.com](https://white.apher.com)

## 📝 Auteursrecht

Uitsluitend voor leer- en tentoonstellingsdoeleinden.
White Studio · 2026

---

*Built with [OpenClaw](https://github.com/openclaw/openclaw)*
