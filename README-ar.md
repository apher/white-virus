# White Virus

> عرض PPSX تفاعلي حول جمالية الفيروس الرقمي

[简体中文](README.md) · [English](README-en.md) · [日本語](README-ja.md) · [한국어](README-ko.md) · [Español](README-es.md) · [Français](README-fr.md) · [Deutsch](README-de.md) · [Português](README-pt.md) · [Italiano](README-it.md) · [Русский](README-ru.md) · [हिंदी](README-hi.md) · [Türkçe](README-tr.md) · [Polski](README-pl.md) · [Nederlands](README-nl.md) · [Tiếng Việt](README-vi.md) · [ไทย](README-th.md) · [Bahasa Indonesia](README-id.md) · [Українська](README-uk.md)

---

**White Virus** هو عرض تفاعلي تتمحور حول مفهوم جمالية "الفيروس الرقمي". من خلال نوافذ الأخطاء في ويندوز، ووميض الشاشة، وقصف النوافذ المنبثقة، يبني تجربة سردية رقمية ذات تأثير كبير.

## 📂 محتويات المشروع

| الملف | الوصف |
|-------|-------|
| `white病毒.ppsx` | ملف PowerPoint Show الأصلي |

## 🚀 البدء السريع

### ⬇️ التنزيل المباشر

**تنزيل** → [white病毒.ppsx](https://github.com/apher/white-virus/releases/download/v1.0/white.ppsx)

### تشغيل PPSX
1. نزّل `white病毒.ppsx`
2. افتحه باستخدام PowerPoint أو WPS، اختر وضع **عرض الشرائح**

## 🌐 التجربة عبر الإنترنت

> **مشغل عبر الإنترنت (pdf.js)** → [https://white.apher.com/white-virus.html](https://white.apher.com/white-virus.html)

> **الصفحة الرئيسية للمشروع** → [https://white.apher.com](https://white.apher.com)

## 🎨 حول المشروع

- **المفهوم الأساسي**: الفيروس الرقمي / جمالية الخلل
- **سيناريوهات الاستخدام**: عرض الفن الرقمي، السرد التفاعلي، التضمين الويب
- **التنسيق**: PPSX (عرض شرائح، غير قابل للتحرير)
- **المعرض** → [https://white.apher.com](https://white.apher.com)

## 📝 حقوق النشر

للأغراض التعليمية والعرض فقط.
White Studio · 2026

---

*Built with [OpenClaw](https://github.com/openclaw/openclaw)*
