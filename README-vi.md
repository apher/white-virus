# White Virus

> Dự án ngoại khóa của bé — bài thuyết trình PPSX tương tác về thẩm mỹ virus kỹ thuật số

[简体中文](README.md) · [English](README-en.md) · [日本語](README-ja.md) · [한국어](README-ko.md) · [Español](README-es.md) · [Français](README-fr.md) · [Deutsch](README-de.md) · [Português](README-pt.md) · [Italiano](README-it.md) · [Русский](README-ru.md) · [العربية](README-ar.md) · [हिंदी](README-hi.md) · [Türkçe](README-tr.md) · [Polski](README-pl.md) · [Nederlands](README-nl.md) · [ไทย](README-th.md) · [Bahasa Indonesia](README-id.md) · [Українська](README-uk.md)

---

**White Virus** là một bài thuyết trình tương tác xoay quanh khái niệm thẩm mỹ "virus kỹ thuật số". Thông qua các cửa sổ lỗi Windows, nhấp nháy màn hình và bom nổi cửa sổ, nó tạo ra một trải nghiệm kể chuyện kỹ thuật số có tác động mạnh mẽ.

## 📂 Nội dung dự án

| Tệp | Mô tả |
|-----|-------|
| `white病毒.ppsx` | Tệp PowerPoint Show gốc |

## 🚀 Bắt đầu nhanh

### ⬇️ Tải trực tiếp

**Tải xuống** → [white病毒.ppsx](https://github.com/apher/white-virus/releases/download/v1.0/white.ppsx)

### Phát PPSX
1. Tải xuống `white病毒.ppsx`
2. Mở bằng PowerPoint hoặc WPS, chọn chế độ **Trình chiếu**

## 🌐 Trải nghiệm trực tuyến

> **Trình phát trực tuyến (pdf.js)** → [https://white.apher.com/white-virus.html](https://white.apher.com/white-virus.html)

> **Trang chủ dự án** → [https://white.apher.com](https://white.apher.com)

## 🎨 Về dự án

- **Auteur**: APher (dự án ngoại khóa của bé)
- **Trường hợp sử dụng**: Triển lãm nghệ thuật số, kể chuyện tương tác, nhúng web
- **Định dạng**: PPSX (Trình chiếu, không thể chỉnh sửa)
- **Bộ sưu tập** → [https://white.apher.com](https://white.apher.com)

## 📝 Bản quyền

Chỉ dành cho mục đích học tập và trưng bày.
White Studio · 2026

---

*Built with [OpenClaw](https://github.com/openclaw/openclaw)*
